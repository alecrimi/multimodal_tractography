...
def main():
    try:
        ...
        if not fns:
            raise ValueError('img_dir ({args.img_dir}) does not contain any .tif or .tiff images.')
        imgs = []
        for fn in fns:
        ...
        img = np.stack(imgs, axis=args.axis)
            '''
            img = tiff.imread(fn)
            nib.Nifti1Image(img,None).to_filename(os.path.join(args.out_dir, base+'.nii.gz'))
        return 0
